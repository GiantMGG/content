/*-- LavaWallLong.c4s — cycle-90 symptom-1 repro (spec §4.0-4.1). --*/
/* Basin: painted fill over a diggable Sandstone floor; step 1     */
/* digs the player-analogue slot; step 9 asserts the drain.        */
/* D3500 long-delay cell: the ONLY Ashes-Instable discriminator.   */
/* cycle-92 MUT-3: drain 0, famB 8771 (the v366 signature) shows   */
/* only at long delay.                                             */
#strict 2

static const FillMatName = "Lava";
static const FamBName = "Ashes";
static const DRAIN_MIN = 20;
static const FALL_MIN = 50;
static const DigDelay = 3500;   // tick at which the slot is dug
...
protected func Initialize()
{
	...
	if (g_iBaseline < 8000)
		FatalError(Format("LavaWallLong FAIL: fill short (%d)", g_iBaseline));
	g_iStep = 0;
	g_fCalibrate = 0;
...
global func FxRunTestTimer(object target, int effect, int timer)
{
	...
	else
	{
		if (GetPXSCount() > 10000)
			FatalError(Format("LavaWallLong FAIL step %d: PXS budget exceeded (%d)", g_iStep, GetPXSCount()));
		if (GetMaterialCount(Material("Steam")) != 0)
			FatalError(Format("LavaWallLong FAIL step %d: static steam appeared (%d)", g_iStep, GetMaterialCount(Material("Steam"))));
	}
	...
	if (g_fCalibrate || do_assert)
	{
		...
		{
			if (drain < DRAIN_MIN)
				FatalError(Format("LavaWallLong FAIL: tank drain %d pct < %d (painted %s frozen)", drain, DRAIN_MIN, FillMatName));
			if (fall < FALL_MIN)
				FatalError(Format("LavaWallLong FAIL: fall-zone family %d < %d", fall, FALL_MIN));
			Log("LavaWallLong PASS");
			GameOver();
			return -1;
		}
	}
