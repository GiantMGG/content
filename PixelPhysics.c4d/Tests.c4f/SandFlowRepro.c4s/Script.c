/*-- SandFlowRepro.c4s — cycle-90 symptom-1 repro (spec §4.0-4.1). --*/
...
protected func Initialize()
{
	...
	if (g_iBaseline < 8000)
		FatalError(Format("SandFlowRepro FAIL: fill short (%d)", g_iBaseline));
	g_iStep = 0;
	g_fCalibrate = 0;
...
global func FxRunTestTimer(object target, int effect, int timer)
{
	...
	else
	{
		if (GetPXSCount() > 10000)
			FatalError(Format("SandFlowRepro FAIL step %d: PXS budget exceeded (%d)", g_iStep, GetPXSCount()));
		if (GetMaterialCount(Material("Steam")) != 0)
			FatalError(Format("SandFlowRepro FAIL step %d: static steam appeared (%d)", g_iStep, GetMaterialCount(Material("Steam"))));
	}
	if (g_fCalibrate)
	...
	if (!g_fCalibrate && g_iStep >= FinalStep)
	{
		...
		if (drain < DRAIN_MIN)
			FatalError(Format("SandFlowRepro FAIL: tank drain %d pct < %d (painted %s frozen)", drain, DRAIN_MIN, FillMatName));
		if (fall < FALL_MIN)
			FatalError(Format("SandFlowRepro FAIL: fall-zone family %d < %d", fall, FALL_MIN));
		Log("SandFlowRepro PASS");
		GameOver();
		return -1;
	}
	return 1;
