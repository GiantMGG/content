...
public func CastlePartAdjust()
{
  ...
  var iWdtH, iHgtH;   // Breite/H�he dieses Teils div 2
  var iAdjustRange;   // CastlePartRange
  // Breite, H�he und Oberkante des auszurichtenden Teils ermitteln
...
public func RecheckBasement()
{
  ...
  // Pr�fen, ob links und rechts ein Burgteil unterhalb existiert
  var iWdt4 = CastlePartWidth()/4;
  var iY = CastlePartHeight()/2 + CastlePartRange();
  // Unterhalb rechts pr�fen
  if (FindCastlePart(iWdt4, iY))
    // Unterhalb links pr�fen
    if (FindCastlePart(-iWdt4, iY))
      // OK; das Fundament ist unn�tig
      RemoveObject(basement);
  // Fertig
  return(1);
}

/* �berpr�fungen bei Ver�nderungen in der Burg */
...
// Der Aufruf von CastleChange muss verz�gert erfolgen, damit das Teil zum Aufruf auch wirklich weg ist
...
// Muss seltsamerweise ebenso verz�gert aufgerufen werden
