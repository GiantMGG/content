/*-- ScorpionStagingSmoke.c4s — cycle-90 symptom-5 staging probe —--*/
...
func RunSmokeSteps()
{
	...
	if (!scorp || !scorp2 || !victim)
		FatalError("ScorpionStagingSmoke FAIL step 0: spawn failed");
	var hfx = GetEffect("WLF_HuntPrey", scorp);
	if (!hfx)
		FatalError("ScorpionStagingSmoke FAIL step 0: SCRP missing WLF_HuntPrey");
	...
	if (!SEqual(GetCommand(scorp), "MoveTo"))
		FatalError("ScorpionStagingSmoke FAIL step 1: hunt issued no MoveTo");
	...
	if (!vfx)
		FatalError("ScorpionStagingSmoke FAIL step 2: sting applied no venom");
	if (GetPhysical("Walk", 0, victim) >= walk0)
		FatalError("ScorpionStagingSmoke FAIL step 2: venom did not cut Walk");
	...
	if (scorp->FxScorpionVenomTimer(victim, vfx, 175) != -1)
		FatalError("ScorpionStagingSmoke FAIL step 3: venom timer did not expire");
	scorp->FxScorpionVenomStop(victim, vfx, 0, false);
	if (GetPhysical("Walk", 0, victim) != walk0)
		FatalError("ScorpionStagingSmoke FAIL step 3: Walk not restored after venom wear-off");

	Log("ScorpionStagingSmoke PASS");
	GameOver();
	return true;
}
