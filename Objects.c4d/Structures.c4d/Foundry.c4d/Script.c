...
func WorkerAcquireFuel (object clonk) {
  AddCommand (clonk, "Call", this (), 0, 0, 0, 0, "Acquisition", 0, 3);
  // Wenn Kohle da (Leider noch keine �berpr�fung auf Kohleherstellendes Objekt)
...
{
  // Hat auch nicht geklappt. Dann halt �lf�sser suchen...
...
private func ContentsCheck()
{
  ...
  // Erstes Inhaltsobjekt �berpr�fen
  obj = Contents(0); var id = GetID(obj);
  // Alles au�er Mannschaftsmitgliedern, Rohmaterial und Loren sofort auswerfen
  if (obj)
